...
local batt_on          = 0 
local cellDataHistoryLowest = { 5, 5, 5, 5, 5, 5 }
...
local function run(voltsource)
  ...
  for k, v in pairs(voltsource) do
    -- stores the lowest cell values in historical table
    if v > 1 and v < cellDataHistoryLowest[k] then
      -- min 1v to consider a valid reading
      cellDataHistoryLowest[k] = v

      --- calc history lowest of all cells
      if v < cellDataHistoryCellLowest then
        cellDataHistoryCellLowest = v
      end
